...
******************************************
	...
	@author: Mason Poe (mason{at--no--spam}skylab{peroid}ws))
	@usage: 
		depth = 0;
		//example1
	...
	@History: released 07.10.04
			...
			updated 08.02.05 -- added localPlayback method.  Allows for caching to be turned on or off.
...
class com.masonpoe.utils.MP3Player{
	...
	if(!this.localPlayback){
		stringAppend = "?id="+Math.round(Math.random()*1000)
	}else{
		stringAppend = ""
	}
	//
	if ((typeof (track)) != "string"){
		...
		//
		soundURL = soundURL+stringAppend
	...
	// set caching.  Tells flash to append url string with a random number.  The string URL is appended by defalt with a random number
	public function set localPlayback(state:Boolean):Void{
	   _localPlayback = state
	}
	// return the state of localPlayback.  Returned as false if not defined by setting localPlayback
	public function get localPlayback():Boolean{
		if(this._localPlayback==undefined){this._localPlayback = false}
		//
